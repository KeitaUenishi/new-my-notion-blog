...
const PromiseAll = () => {
  ...
  const onClickAwait = async () => {
    ...
    const oneAwait = await fetch("/api/dummy/one_time").then((res) => res.json());
    const twoAwait = await fetch("/api/dummy/two_time").then((res) => res.json());
    const threeAwait = await fetch("/api/dummy/three_time").then((res) => res.json());
...
const Container = ({ message, data, setData, onClick }: ContainerProps) => {
  return (
    <div>
      ...
      <Button style={{ marginLeft: "16px" }} variant="contained" color="error" onClick={() => setData([])}>
        クリア
      </Button>
