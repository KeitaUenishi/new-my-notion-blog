  ...
  constructor() {
    return {
      databases: {
        query: () => {
          return JSON.parse(
            fs.readFileSync(path.resolve("./__tests__/fixtures/notion-api-response-pages.json")).toString(),
          );
        },
        retrieve: () => {
          return JSON.parse(
            fs.readFileSync(path.resolve("./__tests__/fixtures/notion-api-response-database.json")).toString(),
          );
        },
      },
      blocks: {
        ...
        retrieve: ({ block_id }) => {
          if (block_id === "359244aa-d0bf-43a0-aa32-87a3afba908d") {
            return JSON.parse(
              fs
                .readFileSync(path.resolve("./__tests__/fixtures/notion-api-response-original-synced-block.json"))
                .toString(),
            );
          }
          return {};
